...
while keeping that list current by itself, gives each deck a page, delivers a
deck's built talk from that page, and lets an admin and every person say how
the lobby looks and which language it speaks. Nothing builds that talk yet, and
nothing is deployed, so no phase of [VISION.md](VISION.md) is reached. M0 is
tracked on
